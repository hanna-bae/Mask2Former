...
@torch.no_grad()
# foreground point -> virtual point 
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=100, dist_thresh=3000, num_camera=1, intrinsics=None, transforms=None, k=5):
    ...
    for idx, k_phase in enumerate(gauss_uniform_ratio):
        ...
        for j in range(k_min):
            ...
            for i in range(num_camera):
                ...
                # virtual point -> 3D point reprojection
                per_camera_virtual_points_3d = reverse_view_points(per_camera_virtual_points_padded, per_camera_virtual_points_depth, intrinsics[i]) #[3, 1200]
    ...
    all_virtual_points = torch.cat([all_virtual_points, all_point_labels], dim=1) # position 3 + class 10 + confidence 1 + radar 4 = 18
...
def process_one_frame(info, predictor, data, num_camera=1):
    # lidar point load -> instance mask -> 2D image projection 
...
if __name__ == "__main__":
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        save_path = './data/tj4d/training/new_3d_mask_2/'
        ...
        if res is not None:
            virtual_points, real_points, indices, prob_map_all = res # virtual_points: position 3 (x, y, z) + class 8 + confidence 1 + radar (RCS, velocity) 5
            #print(virtual_points[0], real_points[0]) 
