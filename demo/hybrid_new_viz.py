...
def augment_with_3d_gaussan(points, num_new_points=50, rcs_scale=1, vel_scale=0.1):
    '''
    Args: 
...
# def augment_with_3d_gaussan(points, num_new_points=20, rcs_scale=0.8, vel_scale=0.1):
#     '''
#     Args:
#         points (np.ndarray): (N, 19) position 3 + intensity 1 + radar 3 + dummy 1 + class 10 + confidence 1
#         num_new_points (int): number of new points to generate
#         rcs_scale (float): scale for RCS
#         vel_scale (float): scale for velocity
#     returns:
...
#         rcs = point[4]        # RCS
#         vx_comp = point[5]    # 속도 x 성분
#         vy_comp = point[6]    # 속도 y 성분

#         # RCS 값에 따른 분산 계산
#         sigma_rcs = rcs_scale * np.log(1 + np.abs(rcs))
#         if sigma_rcs < 1e-6:
#             sigma_rcs = 0.5  # RCS가 너무 낮을 경우 고정 분산 설정

#         # 3D 가우시안 분포의 공분산 행렬
#         cov_matrix = np.diag([sigma_rcs, sigma_rcs, sigma_rcs])
...
if __name__ == "__main__":
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        save_path = './data/viz_result_new_2/'
