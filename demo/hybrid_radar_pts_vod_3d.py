...
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=100, dist_thresh=3000, num_camera=1, intrinsics=None, transforms=None):
    ...
    point_labels = torch.concat([point_labels, raw_points[:, 3:]], dim=1) # 원본 radar 특성 결합 
...
# # add 
# def augment_with_3d_gaussan(points, num_new_points=20, rcs_scale=1, vel_scale=0.1):
#     '''
#     Args: 
#         points (np.ndarray): (N, 18) position 3 + class 8 + zero padding 2 + confidence 1 + radar 4
#         num_new_points (int): number of new points to generate
#         rcs_scale (float): scale for RCS
#         vel_scale (float): scale for velocity
#     returns:
#         position 3 + class 8 + zero padding 2 + confidence 1 + radar 4
#     '''
#     augmented_points = []
#     for point in points:
#         print(point)
...
def process_one_frame(info, predictor, data, num_camera=1):
    ...
    if res is not None:
        ...
        # test 
        # print(f"virtual_points.shape: {virtual_points.shape}") #[3000, 18]
        # print(f"foreground_real_points.shape: {foreground_real_points.shape}") #[94,18]
        # print(foreground_real_points)
        pass
...
if __name__ == "__main__":
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        save_path = './data/vod/training/new_3d_mask_2/'
