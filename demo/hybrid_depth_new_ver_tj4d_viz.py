...
def sample_pixels_from_mask(mask: torch.Tensor,
                            max_pixels_per_mask: int = 1000,
                            stride: int = 4,
                            edge_ratio: float = 0.3):
    """
    mask: (H, W) bool/0-1
    리턴: (M, 2) [y, x]
    tj4d는 vod 보다 절대적 객체 크기 자체가 작아서 제한
    """
    H, W = mask.shape
    # 1) stride 그리드 샘플
    if stride > 1:
...
def add_pixel_virtual_points(
    ...
    max_pixels_per_mask=1000, stride=4, edge_ratio=0.3,
    ...
    #all_virtual = torch.cat([pts_lidar, all_lbls, knn_features], dim=1)
    #print(all_virtual.shape)
...
if __name__ == "__main__":
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        save_path = './data/tj4d/seg_dep_mask_viz/'
