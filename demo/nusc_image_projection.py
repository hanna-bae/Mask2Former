...
# 2D 이미지 좌표와 깊이 정보를 사용해서 3D 공간의 좌표 복원. 
def reverse_view_points(points, depths, view, device='cuda:0'):
    # TODO: write test for reverse projection 
    nbr_points = points.shape[1]
    points  = points * depths.repeat(3, 1).reshape(3, nbr_points) # (x, y, 1) * depth 

    points = torch.cat((points, torch.ones([1, nbr_points]).to(device)), dim=0) # (x, y, z, 1)

    viewpad = torch.eye(4).to(device)
    viewpad[:view.shape[0], :view.shape[1]] = torch.inverse(view) # 2D -> 3D 
...
def projectionV2(points, all_cams_from_lidar, all_cams_intrinsic, H=900, W=1600, device='cuda:0'):
    ...
    num_camera = len(all_cams_from_lidar) # lidar to camera
