...
@torch.no_grad()
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=100, dist_thresh=3000, num_camera=1, intrinsics=None, transforms=None):
    ...
    for idx, k_phase in enumerate(gauss_uniform_ratio):
        ...
        for j in range(k_min):
            ...
            for i in range(num_camera):
                ...
                all_real_points.append(raw_points.reshape(1, -1, 7).repeat(num_camera, 1, 1).reshape(-1, 7)[per_camera_indices])
...
def test_valid(labels):
    ...
    for idx, label in enumerate(labels):
        if label in index:
            output[idx] = 1
    return output > 0.5
def augment_with_3d_gaussan(points, num_new_points=10, rcs_scale=0.2, vel_scale=0.1):
    '''
    Args:
        points (np.ndarray): (N, 18) position 3 + intensity 1 + radar 2 + dummy 1 + class 10 + confidence 1
        num_new_points (int): number of new points to generate
        rcs_scale (float): scale for RCS
        vel_scale (float): scale for velocity
    returns:
        Augmented points as a NumPy array.
    '''
    augmented_points = []
    for point in points:
        # 데이터 구조에 맞게 인덱스 수정
        pos = point[:3]       # 위치 (x, y, z)
        rcs = point[4]        # RCS
        v_comp = point[6]    # 속도 y 성분

        # RCS 값에 따른 분산 계산
        sigma_rcs = rcs_scale * np.log(1 + np.abs(rcs))
        if sigma_rcs < 1e-6:
            sigma_rcs = 0.1  # RCS가 너무 낮을 경우 고정 분산 설정

        # 3D 가우시안 분포의 공분산 행렬
        cov_matrix = np.eye(3) * sigma_rcs 

        # 속도 방향으로의 평균 이동 벡터
        velocity_shift = np.array([v_comp, v_comp, 0]) * vel_scale
        ...
        try:
            ...
            # 공분산 행렬이 양의 정부호(positive definite)가 아닐 경우 처리
            continue

        # 위치 정보 뒤에 나머지 속성(intensity, radar, class, confidence) 추가
        attributes = np.tile(point[3:], (num_new_points, 1))
        new_points = np.hstack((new_points, attributes))
...
if __name__ == "__main__":
    ...
    print('[DEBUG] dataset length: ', len(data_loader.dataset))
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        save_path = './data/vod/training/vod_mask_2/'
        ...
        if res is not None:
            virtual_points, real_points, indices, prob_map_all = res # virtual_points: 位置_3 + 类别_8 + 置信度_1 + 雷达特征_4
            img_path = os.path.join('./data/vod/training/image_2/', info['image']['image_idx'] + '.jpg')
            img = cv2.imread(img_path)
        else:                                          # real_points:    位置_3 + 雷达特征_4 + 类别_8 + 置信度_1
        ...
        def project_points(pts, lidar2cam, cam2img):
            pts_h = np.concatenate([pts, np.ones((pts.shape[0], 1))], axis=1)
            cam_pts = (lidar2cam @ pts_h.T).T
            uvw = (cam2img @ cam_pts[:, :3].T).T
            uv = uvw[:, :2] / uvw[:, 2:3]
            return uv, cam_pts[:, 2] > 0  # z>0만 표시

        uv_virtual, mask_v = project_points(virtual_points[:, :3], lidar2cam, cam2img)
        uv_real, mask_r = project_points(real_points[:, :3], lidar2cam, cam2img)

        for (u, v) in uv_virtual[mask_v]:
            cv2.circle(img, (int(u), int(v)), 2, (0, 0, 255), -1)  # 빨강: virtual
        for (u, v) in uv_real[mask_r]:
            cv2.circle(img, (int(u), int(v)), 2, (0, 255, 0), -1)  # 초록: real

        cv2.imwrite(os.path.join(save_path, f"{info['image']['image_idx']}.png"), img)