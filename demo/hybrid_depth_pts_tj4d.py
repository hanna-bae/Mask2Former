...
def add_virtual_mask(masks, labels, points, raw_points, depth_map, num_virtual=100, depth_thresh=10, num_camera=1, intrinsics=None, transforms=None):
    ...
    for idx, mask in enumerate(masks):
        if len(torch.where(valid[:, idx])[0]) != 0:
            ...
            avg_depth_from_radar = cur_points[:, 2].mean() # z value mean
            #print(mask.shape)
            
            # 3. Find all pixels within the current mask
            mask_indices = mask.reshape(W,H)
            ...
            if num_valid_pixels > 0:
                ...
                # Transform to 3D radar coordinates
                virtual_points_3d_radar = torch.matmul(torch.inverse(transforms[0]),
                                                torch.cat([
                ...
                virtual_points_instance = torch.cat([virtual_points_3d_radar.transpose(1,0), sampled_labels], dim=-1)
                
                virtual_points_list.append(virtual_points_instance)
        else:
            indices = mask.reshpae(W, H).nonzero()
            selected_indices = torch.randperm(len(indices), device=mask.device)[:num_virtual]
...
if __name__ == "__main__":
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        save_path = './data/tj4d/training/depth_mask/'
